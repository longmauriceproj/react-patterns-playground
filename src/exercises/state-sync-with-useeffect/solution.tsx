...
const CartItem = memo(
  ({ product, onUpdateQuantity, onRemove }: CartItemProps) => (
...
const QuantitySelector = memo(
  ({ quantity, onChange, onRemove }: QuantitySelectorProps) => (
...
const StockIndicator = memo(({ inStock, leadTime }: StockIndicatorProps) => (
...
const OrderSummary = memo(
  ({ subtotal, tax, total, onCheckout }: OrderSummaryProps) => (
    <section
...
const ShoppingCartManager = () => {
  ...
  // Best Practice #1: Use useMemo for expensive calculations
  const { subtotal, tax, total } = useMemo(() => {
    ...
    return {
      ...
      tax: taxValue,
      total: subtotalValue + taxValue + SHIPPING_COST,
    };
  }, [products]);

  // Best Practice #2: Use event handlers for state updates when possible. Bonus: useCallback for event handlers
